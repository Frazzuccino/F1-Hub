...
const state = {
  route:'home', schedule:[], drivers:[], constructors:[], photos:{}, news:[], penaltyPoints:{...FALLBACK_POINTS}, reprimands:{...FALLBACK_REPRIMANDS},
...
function radarUrl(r){ const l=r.Circuit.Location; return `https://www.rainviewer.com/?loc=${encodeURIComponent(l.lat)},${encodeURIComponent(l.long)},8&oCS=1&c=2&o=70&layer=radar&sm=1&sn=1`; }

async function loadBase(force=false){
  state.refreshing=true; if(force){['schedule','drivers','constructors','photos','news'].forEach(k=>localStorage.removeItem('f1hub:'+k));}
  try{
...
function setRoute(route){ state.route=route; window.scrollTo({top:0,behavior:'instant'}); document.querySelectorAll('.nav-btn').forEach(b=>b.classList.toggle('active',route===b.dataset.route || (route.includes(':')&&b.dataset.route==='races'))); render(); }
function render(){
  ...
  if(r.startsWith('race:'))return renderRaceDetail(r.split(':')[1]); if(r.startsWith('driver:'))return renderDriver(r.slice(7)); if(r.startsWith('circuit:'))return renderCircuit(r.split(':')[1]);
...
function renderHome(){
  ...
  view.innerHTML=`
    ...
    <div class="grid desktop-two"><div>
      ...
      <div class="spacer"></div><div class="actions"><button class="external-btn red" onclick="setRoute('race:${r.round}')">RACE HUB</button><a class="external-btn" href="${radarUrl(r)}" target="_blank" rel="noopener">RAIN RADAR ↗</a></div>
    </div><div>
...
function renderMore(){ const install=state.installPrompt?`<div class="spacer"></div><button id="install-btn" class="external-btn red">INSTALL F1 HUB</button>`:''; view.innerHTML=titleBlock('F1 HUB','Explore')+`<div class="menu-grid">
  ...
  </div>${install}<div class="source-note">F1 Hub is a personal unofficial Formula 1 companion. Live/current data comes from free public sources and is cached locally.</div>`;
...
function driverCard(s){ const team=s.Constructors?.at(-1)?.name||'',p=state.photos[driverCode(s.Driver)],img=p?.headshot_url;return `<div class="card driver-card clickable" onclick="setRoute('driver:${s.Driver.driverId}')"><i class="team-strip" style="background:${teamColour(team)}"></i>${img?`<img class="driver-photo" src="${esc(img)}" alt="${esc(fullName(s.Driver))}" loading="lazy">`:`<div class="avatar">${esc(driverCode(s.Driver))}</div>`}<div class="driver-copy"><div class="driver-code">#${esc(s.Driver.permanentNumber||'—')} · ${esc(driverCode(s.Driver))}</div><div class="driver-full">${esc(fullName(s.Driver))}</div><div class="driver-bottom"><span>${esc(team)}</span><span><b>${esc(s.points)}</b> pts</span></div></div></div>`; }
function renderDriver(id){ const s=state.drivers.find(x=>x.Driver.driverId===id);if(!s)return setRoute('drivers');const d=s.Driver,team=s.Constructors?.at(-1)?.name||'',p=state.photos[driverCode(d)],img=p?.headshot_url;view.innerHTML=`<div class="actions"><button class="external-btn" onclick="setRoute('drivers')">← DRIVERS</button></div><div class="spacer"></div><div class="card" style="overflow:hidden"><div style="display:grid;grid-template-columns:1fr 1fr;align-items:end;background:linear-gradient(135deg,#181818,#0d0d0d);border-left:6px solid ${teamColour(team)}"><div style="padding:20px"><div class="eyebrow">#${esc(d.permanentNumber||'—')} · ${esc(driverCode(d))}</div><h1 style="font-size:34px;margin:6px 0">${esc(fullName(d))}</h1><div class="muted">${esc(d.nationality)} · ${esc(team)}</div></div>${img?`<img src="${esc(img)}" alt="" style="width:100%;max-height:245px;object-fit:contain;object-position:right bottom">`:''}</div><div style="padding:14px"><div class="facts"><div class="fact"><b>${esc(s.position)}</b><small>CHAMP POS</small></div><div class="fact"><b>${esc(s.points)}</b><small>POINTS</small></div><div class="fact"><b>${esc(s.wins)}</b><small>WINS</small></div><div class="fact"><b>${age(d.dateOfBirth)}</b><small>AGE</small></div><div class="fact"><b>${fmtDate(d.dateOfBirth+'T12:00:00Z',{day:'numeric',month:'short',year:'numeric'})}</b><small>BORN</small></div><div class="fact"><b>${esc(d.nationality)}</b><small>NATIONALITY</small></div></div></div></div>`; }
...
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();state.installPrompt=e;if(state.route==='more')renderMore();});
